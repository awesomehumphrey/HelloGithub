...
for (y in 1:runs){
  ...
  tol = sample(tolerance,start,replace = TRUE)#choosing class of viewers at random
  ...
  for (x in 1:start){  
    hop[x] = abs(((len[x]*nbrk[x]*nadvert[x])/(tol[x]*nmovie[x]))-gluef[x])# the absolute function is to ensure all values are positive
