...
carAdverts = c(ford = 0.09, nissan = 0.15, bmw = 0.22, Maz = 0.39)#adverts with glue factors
...
for (x in startpoint){
  ...
  hop[x] = ((lenfilm[1]*brkprob[10]*advprob[4])/(teenprob*movie[3]))-carAdverts[4]
...
print (counthop)
print (countabs)
((lenfilm[1]*brkprob[10]*advprob[4])/(teenprob*movie[6]))-carAdverts[4]
